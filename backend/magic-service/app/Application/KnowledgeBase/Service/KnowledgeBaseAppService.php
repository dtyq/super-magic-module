...
class KnowledgeBaseAppService extends AbstractKnowledgeAppService
{
    ...
    public function save(Authenticatable $authorization, KnowledgeBaseEntity $magicFlowKnowledgeEntity, array $documentFiles = []): KnowledgeBaseEntity
    {
        ...
        try {
            ...
            $embeddingResult = $embeddingModel->embeddings('test', businessParams: ['organization_id' => $dataIsolation->getCurrentOrganizationCode(), 'user_id' => $dataIsolation->getCurrentUserId()]);
            if (count($embeddingResult->getData()[0]?->getEmbedding()) !== $embeddingModel->getVectorSize()) {
