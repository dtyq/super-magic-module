...
class MagicDepartmentAppService extends AbstractAppService
{
    ...
    public function getSubDepartments(DepartmentQueryDTO $queryDTO, MagicUserAuthorization $userAuthorization): array
    {
        ...
        if ($departmentId === PlatformRootDepartmentId::Magic) {
            $departments = $this->magicDepartmentDomainService->getSubDepartmentsByLevel($dataIsolation, 0, 1, $pageSize, $offset);
        } else {
            // 获取部门
            $departments = $this->magicDepartmentDomainService->getSubDepartmentsById($dataIsolation, $departmentId, $pageSize, $offset);
        }
    ...
    public function searchDepartment(DepartmentQueryDTO $queryDTO, MagicUserAuthorization $userAuthorization): array
    {
        ...
        $departments = $this->filterDepartmentsHidden($departments);
        // 全量查找，没有更多
        return PageListAssembler::pageByMysql($departments, (int) $pageToken);
    }
