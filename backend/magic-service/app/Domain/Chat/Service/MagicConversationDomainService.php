...
class MagicConversationDomainService extends AbstractDomainService
{
    ...
    public function getConversations(DataIsolation $dataIsolation, ConversationListQueryDTO $queryDTO): array
    {
        $conversationDTO = new MagicConversationEntity();
        $conversationDTO->setUserId($dataIsolation->getCurrentUserId());
        // 该账号的所有用户id
        $userEntities = $this->magicUserRepository->getUserAllUserIds($dataIsolation->getCurrentUserId());
        $userIds = array_column($userEntities, 'user_id');
        if (! empty($queryDTO->getOrganizationCode())) {
            $userIds = [];
            foreach ($userEntities as $userEntity) {
                if ($userEntity->getOrganizationCode() === $queryDTO->getOrganizationCode()) {
                    $userIds = [$userEntity->getUserId()];
                }
            }
        }

        return $this->magicConversationRepository->getConversationsByUserIds($conversationDTO, $queryDTO, $userIds);
    }
