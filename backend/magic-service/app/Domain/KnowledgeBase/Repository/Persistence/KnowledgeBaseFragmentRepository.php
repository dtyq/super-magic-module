...
use App\Domain\KnowledgeBase\Entity\ValueObject\DocumentFileDTO;
...
class KnowledgeBaseFragmentRepository extends KnowledgeBaseAbstractRepository implements KnowledgeBaseFragmentRepositoryInterface
{
    ...
    public function destroyByMetadataFilter(KnowledgeBaseDataIsolation $dataIsolation, string $knowledgeBaseCode, array $metadataFilter): void
    {
        $this->knowledgeBaseFragmentsModel->where('knowledge_code', $knowledgeBaseCode)
            ->where('metadata', 'like', '%' . json_encode($metadataFilter) . '%')
            ->delete();
    }
     ...
     */
    public function fragmentPreview(KnowledgeBaseDataIsolation $dataIsolation, DocumentFileDTO $documentFile, array $fragmentConfig = []): array
    {
        $fileContent = $this->fileDomainService->getFileContent($dataIsolation, $documentFile->getKey());
        $fragments = $this->splitContentIntoFragments($fileContent, $fragmentConfig);

        $list = [];
        foreach ($fragments as $index => $fragment) {
            $entity = new KnowledgeBaseFragmentEntity();
            $entity->setContent($fragment);
            $entity->setMetadata([
                'file_name' => $documentFile->getName(),
                'file_key' => $documentFile->getKey(),
    ...
    private function splitContentIntoFragments(string $content, array $config = []): array
    {
        ...
        switch ($type) {
            case 'paragraph':
                ...
                foreach ($paragraphs as $paragraph) {
                    ...
                    if (mb_strlen($paragraph) > $maxLength) {
                        ...
                        foreach ($sentences as $sentence) {
                            if (mb_strlen($currentFragment . $sentence) > $maxLength) {
                                if ($currentFragment !== '') {
                                ...
                                if (mb_strlen($sentence) > $maxLength) {
                                    $fragments = array_merge($fragments, $this->splitByLength($sentence, $maxLength, $minLength, $overlap));
                                } else {
                ...
                break;
            case 'sentence':
                $fragments = $this->splitBySentences($content);
                break;
            case 'word':
                $fragments = $this->splitByLength($content, $maxLength, $minLength, $overlap);
                break;
        }
    ...
    private function splitBySentences(string $content): array
    {
        // 按句子分割，支持中文和英文标点
        $pattern = '/(?<=[。！？.!?])\s*/u';
        $sentences = preg_split($pattern, $content, -1, PREG_SPLIT_NO_EMPTY);
        return array_map('trim', $sentences);
    }

    private function splitByLength(string $content, int $maxLength, int $minLength, int $overlap): array
    {
        $fragments = [];
        ...
        while ($start < $length) {
            ...
            $fragment = mb_substr($content, $start, $end - $start);

            // 如果剩余内容不足最小长度，且不是最后一段，则合并到前一段
            if ($end < $length && mb_strlen($fragment) < $minLength && ! empty($fragments)) {
