...
class MagicDepartmentDomainService extends AbstractContactDomainService
{
     ...
     * @return MagicDepartmentEntity[]
     */
    public function getSubDepartmentsByLevel(DataIsolation $dataIsolation, int $currentDepartmentLevel, int $depth, int $size, int $offset): array
    {
        $orgCode = $dataIsolation->getCurrentOrganizationCode();
        $departments = $this->departmentRepository->getSubDepartmentsByLevel($currentDepartmentLevel, $orgCode, $depth, $size, $offset);
        return $this->getDepartmentsHasChild($departments, $orgCode);
    }

    /**
     * @return MagicDepartmentEntity[] $departments
     */
    public function getSubDepartmentsById(DataIsolation $dataIsolation, string $departmentId, int $size, int $offset): array
    {
        $orgCode = $dataIsolation->getCurrentOrganizationCode();
        $departments = $this->departmentRepository->getSubDepartmentsById($departmentId, $orgCode, $size, $offset);
        return $this->getDepartmentsHasChild($departments, $orgCode);
    }
