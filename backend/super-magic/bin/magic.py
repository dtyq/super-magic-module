#!/usr/bin/env python
"""
v6 命令行界面，用于调用 Agent 类

使用方法:
    python -m bin.v6 [--agent-name AGENT_NAME] [--clean] [--clean-chat] [--clean-workspace] [--mount DIRECTORY_PATH] [--mode MODE] [查询文本]
    或者直接执行: ./bin/v6.py [--agent-name AGENT_NAME] [--clean] [--clean-chat] [--clean-workspace] [--mount DIRECTORY_PATH] [--mode MODE] [查询文本]

参数:
    --agent-name: 指定要使用的 agent 名称，默认为 "test"
...
from app.utils.file_system_utils import clear_directory_contents
...
async def clean_chat_history():
    ...
    Returns:
        bool: 操作是否成功
    """
    return await clear_directory_contents(PathManager.get_chat_history_dir())
...
async def clean_workspace():
    ...
    Returns:
        bool: 操作是否成功
    """
    return await clear_directory_contents(PathManager.get_workspace_dir())
...
async def clean_directories():
    ...
    chat_result, workspace_result = await asyncio.gather(
        clean_chat_history(),
        clean_workspace(),
        return_exceptions=True # 如果一个失败，另一个继续执行
    )

    # 处理可能发生的异常
    chat_success = True
    if isinstance(chat_result, Exception):
        logger.error(f"清理 chat history 时发生错误: {chat_result}", exc_info=chat_result)
        chat_success = False
    elif not chat_result: # 如果 clear_directory_contents 返回 False
        logger.error("清理 chat history 失败 (详见先前日志)")
        chat_success = False

    workspace_success = True
    if isinstance(workspace_result, Exception):
        logger.error(f"清理 workspace 时发生错误: {workspace_result}", exc_info=workspace_result)
        workspace_success = False
    elif not workspace_result: # 如果 clear_directory_contents 返回 False
        logger.error("清理 workspace 失败 (详见先前日志)")
        workspace_success = False

    return chat_success and workspace_success
