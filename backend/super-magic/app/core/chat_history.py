...
from app.core.chat_history_compressor import ChatHistoryCompressor
...
class ChatHistory:
    ...
    def tokens_count(self) -> int:
        ...
        for i, msg in enumerate(self.messages):
            ...
            if isinstance(msg, AssistantMessage) and msg.token_usage is not None:
                # 对于 AssistantMessage，只累加 completion_tokens 或 prompt_tokens 之一
                # 避免重复计算
                if msg.token_usage.completion_tokens is not None:
                    msg_tokens += msg.token_usage.completion_tokens
                elif msg.token_usage.prompt_tokens is not None:
                    msg_tokens += msg.token_usage.prompt_tokens

                # 缓存相关的tokens不计入总量，只记录用于统计
                # 这些tokens已经包含在completion_tokens或prompt_tokens中
                ...
                try:
                    ...
                    if isinstance(msg, AssistantMessage):
                        if msg.token_usage is None:
                            msg.token_usage = TokenUsageInfo()

                        # 为AssistantMessage设置completion_tokens
                        msg.token_usage.completion_tokens = msg_tokens
                ...
                for msg_dict in history_data:
                    ...
                    try:
                        ...
                        if role == "system":
                            ...
                            message = UserMessage(**args_dict)
                        elif role == "assistant":
                            # ... (AssistantMessage 特殊处理 tool_calls，之前已放入 args_dict) ...
                            tool_calls_data = args_dict.get("tool_calls")
                            tool_calls = None
                            if isinstance(tool_calls_data, list):
                                tool_calls = []
                                for tc_dict in tool_calls_data:
                                    if isinstance(tc_dict, dict) and "function" in tc_dict and isinstance(tc_dict["function"], dict):
                                        func_dict = tc_dict["function"]
                                        if not isinstance(func_dict.get("arguments"), str):
                                            func_dict["arguments"] = json.dumps(func_dict.get("arguments", {}), ensure_ascii=False)
                                        function_call = FunctionCall(**func_dict)
                                        if "id" in tc_dict and "type" in tc_dict:
                                             tool_call_args = {"id": tc_dict["id"], "type": tc_dict["type"], "function": function_call}
                                             tool_calls.append(ToolCall(**tool_call_args))
                                        else: logger.warning(f"加载历史时跳过缺少 id 或 type 的 tool_call: {tc_dict}")
                                    else: logger.warning(f"加载历史时跳过无效的 tool_call 结构: {tc_dict}")
                            args_dict["tool_calls"] = tool_calls # 更新处理后的 tool_calls
                            # AssistantMessage 特有字段 (token_usage)
                            token_usage_data = msg_dict.get("token_usage")
                            token_usage_obj = None
                            if isinstance(token_usage_data, dict):
                                try:
                                    # 从字典创建 TokenUsageInfo，过滤掉非预期字段，处理类型
                                    valid_token_args = {}
                                    for field_name in TokenUsageInfo.__dataclass_fields__:
                                        if field_name in token_usage_data:
                                             raw_val = token_usage_data[field_name]
                                             try:
                                                 valid_token_args[field_name] = int(raw_val) if raw_val is not None else None
                                             except (ValueError, TypeError):
                                                  logger.warning(f"加载历史时 token_usage 字段 '{field_name}' 值无效: {raw_val}，将忽略。")
                                    token_usage_obj = TokenUsageInfo(**valid_token_args)
                                    # 如果创建后所有字段都是 None，则不设置
                                    if token_usage_obj.is_empty():
                                        token_usage_obj = None
                                except Exception as e:
                                    logger.warning(f"加载历史时创建 TokenUsageInfo 失败: {token_usage_data}, 错误: {e}")
                            args_dict["token_usage"] = token_usage_obj
                            message = AssistantMessage(**args_dict)
                        elif role == "tool":
                            message = ToolMessage(**args_dict)
                        else:
    ...
    def save(self) -> None:
        ...
        try:
            ...
            for message in self.messages:
                ...
                # 2. 移除值为默认值的可选字段
                # 检查 show_in_ui (默认 True)
                if msg_dict.get('show_in_ui') is True:
                    msg_dict.pop('show_in_ui')

                # 检查 content (仅 Assistant 可为 None)
                if isinstance(message, AssistantMessage) and msg_dict.get('content') is None:
                    msg_dict.pop('content')

                # 检查 tool_calls (默认 None)
                if isinstance(message, AssistantMessage) and msg_dict.get('tool_calls') is None:
                     msg_dict.pop('tool_calls')

                # 检查 system (仅 ToolMessage 可有, 默认 None)
                if isinstance(message, ToolMessage) and msg_dict.get('system') is None:
                     msg_dict.pop('system')

                # 检查 token_usage (嵌套结构)
                if isinstance(message, AssistantMessage):
                    token_usage_data = msg_dict.get('token_usage')
                    if token_usage_data is not None and isinstance(token_usage_data, dict):
                        # 创建一个新的字典，只包含 token_usage_data 中值不为 None 的字段
                        cleaned_token_usage = {
                            k: v for k, v in token_usage_data.items() if v is not None
                        }
                        # 如果清理后字典为空，则从 msg_dict 中移除 token_usage
                        if not cleaned_token_usage:
                            msg_dict.pop('token_usage')
                        # 否则，用清理后的字典更新 msg_dict 中的 token_usage
                        else:
                            msg_dict['token_usage'] = cleaned_token_usage
                    elif token_usage_data is None:
                         # 如果原始字典中就没有 token_usage，无需操作
                         pass
                    else:
                         # 如果 token_usage 存在但不是字典 (理论上不应发生，防御性移除)
                         logger.warning(f"保存时发现 token_usage 不是字典: {token_usage_data}，将移除。")
                         msg_dict.pop('token_usage', None)

                # duration_ms 已在步骤 1 中移除
    ...
    async def append_assistant_message(self,
                                 ...
                                 # --- 接收 token 数据的字典 ---
                                 token_usage_data: Optional[Dict[str, int]] = None
                                 ) -> None:
        """
        添加一条助手消息。
        ...
        Args:
            ...
            token_usage_data (Optional[Dict[str, int]]): 包含 token 使用信息的字典，
                键应为 TokenUsageInfo 的字段名 (如 'prompt_tokens', 'completion_tokens' 等)。
        """
        ...
        # --- 处理 token_usage_data ---
        token_usage_obj: Optional[TokenUsageInfo] = None
        if token_usage_data and isinstance(token_usage_data, dict):
            try:
                # 从字典创建 TokenUsageInfo，只取有效字段
                valid_args = {
                    k: v for k, v in token_usage_data.items()
                    if k in TokenUsageInfo.__dataclass_fields__ and v is not None
                }
                if valid_args: # 只有当提供了至少一个有效的 token 值时才创建对象
                    token_usage_obj = TokenUsageInfo(**valid_args)
                    # 再次检查是否创建了空对象 (虽然前面过滤了 None, 但以防万一)
                    if token_usage_obj.is_empty():
                         token_usage_obj = None
            except Exception as e:
                logger.warning(f"创建 TokenUsageInfo 失败: {token_usage_data}, 错误: {e}")
                token_usage_obj = None # 创建失败则不设置
