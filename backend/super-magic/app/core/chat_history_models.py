...
class TokenUsageInfo:
    """封装 LLM 调用的 Token 使用量信息"""
    prompt_tokens: Optional[int] = None
    completion_tokens: Optional[int] = None
    cached_tokens: Optional[int] = None # Qwen/DeepSeek 格式
    cache_write_tokens: Optional[int] = None # Claude 格式
    cache_hit_tokens: Optional[int] = None # Claude 格式

    # 辅助方法：检查是否所有字段都为 None
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'TokenUsageInfo':
        """从字典创建TokenUsageInfo对象"""
        return cls(
...
class CompressionConfig:
    ...
    llm_model_for_compression: str = "gpt-4.1-mini"  # 用于压缩的LLM模型
    ...
    def _calculate_model_based_threshold(self) -> int:
        ...
        try:
            ...
            if self.agent_model_id:
                # 从模型配置中获取context_length
                model_config = model_configs.get(self.agent_model_id, {})
                context_length = int(model_config.get("context_length", 0))
                # 设置为上下文长度的70%作为阈值
                threshold = int(context_length * 0.7)
...
class AssistantMessage:
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            ...
            "compression_info": self.compression_info.to_dict() if self.compression_info else None,
        }
        if self.tool_calls:
            result["tool_calls"] = [tc.to_dict() for tc in self.tool_calls]
        if hasattr(self, 'function_call') and self.function_call:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AssistantMessage":
        ...
        if "token_usage" in data and data["token_usage"]:
            from_dict_method = getattr(TokenUsageInfo, "from_dict", None)
            if callable(from_dict_method):
                msg.token_usage = TokenUsageInfo.from_dict(data["token_usage"])
            else:
                # 兼容性处理：如果没有from_dict方法
                msg.token_usage = TokenUsageInfo(**data["token_usage"])

        # 设置压缩信息
        if "compression_info" in data and data["compression_info"]:
            from_dict_method = getattr(CompressionInfo, "from_dict", None)
            if callable(from_dict_method):
                msg.compression_info = CompressionInfo.from_dict(data["compression_info"])
            else:
                # 兼容性处理
                msg.compression_info = CompressionInfo(**data["compression_info"])
        ...
        if "function_call" in data and data["function_call"]:
            function_data = data["function_call"]
            if not hasattr(msg, 'function_call'):
                setattr(msg, 'function_call', FunctionCall(
                    name=function_data.get("name", ""),
                    arguments=function_data.get("arguments", "{}")
                ))
