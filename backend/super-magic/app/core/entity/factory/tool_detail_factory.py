...
class ToolDetailFactory:
    ...
    @staticmethod
    def create_filebase_search_detail(search_results: List[Dict[str, Any]], query: str) -> ToolDetail:
        """
        从Filebase搜索结果创建搜索详情

        Args:
            search_results: 文件搜索结果列表
            query: 搜索查询

        Returns:
            ToolDetail: 工具详情对象
        """
        content = f"文件向量搜索: {query}\n结果数量: {len(search_results)}\n" + json.dumps(
            search_results, ensure_ascii=False, indent=2
        )

        return ToolDetail(
            type=DisplayType.TEXT, data=FileContent(file_name=f"filebase_search_{query}.txt", content=content)
        )

    @staticmethod
    def create_reasoning_detail(title: str, reasoning_content: str, content: str) -> ToolDetail:
        """创建推理类型的工具详情
