...
from app.core.chat_history_models import (
    ChatMessage, AssistantMessage, CompressionConfig, CompressionInfo,
...
class ChatHistoryCompressor:
    ...
    def _filter_messages_to_compress(self,
        ...
        logs_dir = os.path.join(PathManager.get_chat_history_dir(), 'compressed')
...
压缩后的摘要(直接输出):"""
    ...
    async def _call_llm_for_compression(self, system_prompt: str, user_prompt: str) -> Optional[str]:
        ...
        try:
            ...
            # 从配置中获取模型ID
            model_id = self.config.llm_model_for_compression
            ...
            logger.exception(f"调用LLM进行压缩时出错: {repr(e)}")
            return None
    ...
    def _count_message_tokens(self, message) -> int:
        ...
        try:
            ...
            # 计算失败时给出警告并返回估计值
            logger.warning(f"计算消息token数量失败: {str(e)}")
