...
class DownloadFromUrlParams(BaseToolParams):
    ...
    file_path: str = Field(
        ...,
        description="保存文件的路径，相对于工作目录或绝对路径，不要包含工作目录，如果希望将文件保存到 .workspace/webview_reports/file.txt，只需要传入 webview_reports/file.txt 即可"
    )
    filename: Optional[str] = Field(
        None,
        description="自定义文件名，建议提供一个友好的命名便于后续管理，如果提供则使用此名称替换原始文件名，保持扩展名不变。不包含路径信息，仅文件名部分，如：小米财报分析.pdf"
    )
    override: bool = Field(
        False,
...
class DownloadFromUrl(AbstractFileTool[DownloadFromUrlParams], WorkspaceGuardTool[DownloadFromUrlParams]):
    ...
    这个工具可以从HTTP/HTTPS URL下载文件，并支持自动处理重定向。
    """

    # 设置参数类
    params_class = DownloadFromUrlParams

    # 设置工具元数据
    name = "download_from_url"
    description = """从URL下载文件工具
- 支持自动处理重定向
...
"""
    ...
    async def execute(self, tool_context: ToolContext, params: DownloadFromUrlParams) -> ToolResult:
        ...
        try:
            ...
            # 如果提供了自定义文件名，则处理文件路径
            if params.filename:
                # 处理自定义文件名，确保安全性并保留扩展名
                safe_name = get_safe_filename(params.filename)

                # 优雅地处理扩展名
                # 从安全文件名中提取扩展名
                safe_name_ext = os.path.splitext(safe_name)[1].lower()
                # 从原始路径提取扩展名
                original_ext = file_path.suffix.lower()

                # 只有当原始文件有扩展名且安全文件名没有相同扩展名时，才添加扩展名
                if original_ext and safe_name_ext != original_ext:
                    # 如果安全文件名已有扩展名，先移除
                    if safe_name_ext:
                        safe_name = safe_name[:-len(safe_name_ext)]
                    # 添加原始扩展名
    ...
    async def get_tool_detail(self, tool_context: ToolContext, result: ToolResult, arguments: Dict[str, Any] = None) -> Optional[ToolDetail]:
        ...
        if not arguments or "url" not in arguments or "file_path" not in arguments:
            logger.warning("没有提供url或file_path参数")
            return None
    ...
    async def get_after_tool_call_friendly_action_and_remark(self, tool_name: str, tool_context: ToolContext, result: ToolResult, execution_time: float, arguments: Dict[str, Any] = None) -> Dict:
        ...
        # 如果使用了自定义文件名，我们将在备注中显示这些信息
        remark = file_name
        if custom_filename and url:
            # 尝试从URL中提取原始文件名
            try:
                ...
                if original_filename:
                    remark = f"{file_name} (自定义名称，原始: {original_filename})"
            except Exception:
                pass
