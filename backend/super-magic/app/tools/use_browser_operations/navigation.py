...
from app.tools.magic_use.magic_browser import (
    MagicBrowser, GotoSuccess, ScrollSuccess, ScrollToSuccess, MagicBrowserError
)
...
class NavigationOperations(OperationGroup):
    ...
    async def scroll(self, browser: MagicBrowser, params: ScrollParams) -> ToolResult:
        ...
        page, error_result = await self._get_validated_page(browser, params)
        if error_result: return error_result
        # 确保 page_id 存在，因为 scroll_page 需要它
        page_id = params.page_id or await browser.get_active_page_id()
        if not page_id: # 双重保险
            return ToolResult(error="无法确定要滚动的页面ID")

        # 2. 调用 MagicBrowser 的 scroll_page 方法
        try:
            result = await browser.scroll_page(
                page_id=page_id,
            ...
            if isinstance(result, MagicBrowserError):
                ...
                # 格式化成功结果
                scroll_dir = result.direction.upper()
                scroll_type = "整页" if result.full_page else "部分"
                markdown_content = (
                    f"**操作: scroll**\n"
                    f"状态: 成功 ✓\n"
                    f"方向: {scroll_dir}\n"
                    ...
                    f"实际滚动距离: {result.actual_distance:.0f}px"
                )
                return ToolResult(content=markdown_content)
            else:
