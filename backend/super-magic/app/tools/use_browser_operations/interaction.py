...
from app.tools.magic_use.magic_browser import (
    ...
    ClickSuccess, InputSuccess, InteractiveElementsSuccess, ScreenshotSuccess, ScrollSuccess, ScrollToSuccess
)
...
class ScrollParams(BaseOperationParams):
    """滚动页面参数"""
    ...
    full_page: bool = Field(
        False,
        description="是否滚动整页距离 (对 'up'/'down'/'left'/'right' 有效) 或滚动到顶部/底部"
    )
...
class InteractionOperations(OperationGroup):
                    ...
                    for category_name in categories_to_display:
                        ...
                        if elements_in_category and display_count < MAX_DISPLAY:
                            ...
                            for elem in elements_in_category:
                                ...
                                if elem_text:
                                    description += f" | 文本: '{elem_text}{'...' if len(elem.get('text', '')) > 30 else ''}'"
                                elif elem_name != "无名":
                                    description += f" | 名称: '{elem_name}'"

                                if elem_value is not None:
                                    description += f" | 值: '{str(elem_value)[:30]}{'...' if len(str(elem_value)) > 30 else ''}'"
                                if elem_href:
                                    description += f" | 链接: '{elem_href[:50]}{'...' if len(elem_href) > 50 else ''}'"
        ...
        try:
            ...
            for marker_id in marker_ids:
                ...
                (() => {{
                    ...
                    let elementText = (element.textContent || element.innerText || "").trim().substring(0, 100); // 限制长度
            ...
            if found_elements:
                ...
                for i, elem in enumerate(found_elements, 1):
                    ...
                    if elem_text:
                        markdown_content += f"- 文本: '{elem_text}{'...' if len(elem_text) == 100 else ''}'\n"
                    if elem_name:
    ...
    async def scroll(self, browser: MagicBrowser, params: ScrollParams) -> ToolResult:
        """滚动当前页面。

        可以按方向滚动 ('up', 'down', 'left', 'right')，
        也可以直接滚动到页面顶部 ('top') 或底部 ('bottom')。
        'full_page' 参数控制是否滚动整页距离，如：滚动到顶部/底部
        """
        page_id = params.page_id
        if not page_id:
            page_id = await browser.get_active_page_id()
        ...
        try:
            logger.info(f"尝试滚动页面 {page_id}，方向: {direction}, 整页: {full_page}")
            result = await browser.scroll_page(page_id, direction, full_page)

            if isinstance(result, MagicBrowserError):
                return ToolResult(error=result.error)
            elif isinstance(result, ScrollSuccess):
                # 滚动成功后，等待网络稳定，处理懒加载情况
                try:
                    logger.info(f"滚动操作成功，等待页面 {page_id} 网络稳定...")
                    # 增加 5 秒超时，避免无限等待
                    await browser.wait_for_load_state(page_id, state='networkidle', timeout=5000)
                    logger.info(f"页面 {page_id} 网络已稳定。")
                except Exception as wait_e:
                    # 等待超时或失败不是致命错误，记录警告即可
                    logger.warning(f"等待页面 {page_id} 网络稳定时出错或超时: {wait_e!s}")
                ...
                markdown_content = (
                    ...
                    f"状态: 成功 ✓\n"
                    f"方向: {direction}\n"
                    f"整页滚动: {'是' if full_page else '否'}\n"
                    f"滚动前位置: (x={before.get('x', '?')}, y={before.get('y', '?')})\n"
                    f"滚动后位置: (x={after.get('x', '?')}, y={after.get('y', '?')})\n"
                )
                if direction in ['up', 'down', 'left', 'right']:
                    markdown_content += f"实际滚动距离: {abs(actual_distance):.0f} 像素\n"

                # 添加提示信息
                if direction == 'down' and after.get('y', -1) == before.get('y', -2):
                    markdown_content += "\n**提示**: 可能已滚动到底部。\n"
                elif direction == 'up' and after.get('y', -1) == before.get('y', -2):
                    markdown_content += "\n**提示**: 可能已滚动到顶部。\n"

                return ToolResult(content=markdown_content)
            else:
                logger.error(f"scroll 操作返回了未知类型: {type(result)}")
                return ToolResult(error="scroll 操作返回了意外的结果类型。")

        except Exception as e:
            logger.error(f"scroll 外部处理失败: {e!s}", exc_info=True)
            return ToolResult(error=f"滚动页面时发生意外错误: {e!s}")
