...
from app.tools.magic_use.magic_browser import (
...
class GetPageStateParams(BaseOperationParams):
    """获取页面状态的参数 (只是 BaseOperationParams)"""
    pass
...
class TakeScreenshotParams(BaseOperationParams):
    """截图操作的参数"""
    path: Optional[str] = Field(None, description="截图保存的绝对或相对路径。如果省略，将保存到临时目录。")
    full_page: bool = Field(False, description="是否截取整个可滚动页面 (可能很长)")
...
class ContentOperations(OperationGroup):
    ...
    @operation(
        example={
            "operation": "get_page_state"
        }
    )
    async def get_page_state(self, browser: MagicBrowser, params: GetPageStateParams) -> ToolResult:
        """获取当前活动页面的状态信息，包括URL、标题、滚动位置等。"""
        # 1. 获取并验证页面ID
        page_id = params.page_id or await browser.get_active_page_id()
        if not page_id:
            return ToolResult(error="没有活动的页面可获取状态。")

        # 2. 调用 MagicBrowser 的 get_page_state 方法
        try:
            result = await browser.get_page_state(page_id=page_id)

            # 3. 处理返回结果
            if isinstance(result, MagicBrowserError):
                return ToolResult(error=result.error)
            elif isinstance(result, PageStateSuccess):
                state = result.state
                # 格式化成功结果
                content = f"**操作: get_page_state**\n状态: 成功 ✓\n页面ID: {state.page_id}\n"
                if state.url: content += f"URL: `{state.url}`\n"
                if state.title: content += f"标题: {state.title}\n"
                if state.context_id: content += f"上下文ID: {state.context_id}\n"

                if state.position_info:
                    content += f"阅读进度: {state.position_info.read_percent:.0f}%\n"
                    content += f"剩余内容: {state.position_info.remaining_percent:.0f}%\n"
                    content += f"当前屏/总屏数: {state.position_info.current_screen:.0f}/{state.position_info.total_screens:.0f}\n"

                # if state.scroll_position:
                #     content += f"滚动位置 (x,y): ({state.scroll_position.x:.0f}, {state.scroll_position.y:.0f})\n"
                #     content += f"视口尺寸 (宽x高): {state.scroll_position.viewport_width:.0f} x {state.scroll_position.viewport_height:.0f}\n"
                #     content += f"文档尺寸 (宽x高): {state.scroll_position.document_width:.0f} x {state.scroll_position.document_height:.0f}\n"

                return ToolResult(content=content.strip())
            else:
                logger.error(f"get_page_state 操作返回了未知类型: {type(result)}")
                return ToolResult(error="get_page_state 操作返回了意外的结果类型。")

        except Exception as e:
            logger.error(f"get_page_state 外部处理失败: {e!s}", exc_info=True)
            return ToolResult(error=f"获取页面状态时发生意外错误: {e!s}")
    ...
    @operation(
        example={
            ...
            "operation_params": {
                "full_page": True
                # "path": "/path/to/save/screenshot.png" # Optional
            }
        }
    )
    async def take_screenshot(self, browser: MagicBrowser, params: TakeScreenshotParams) -> ToolResult:
        """截取当前活动页面的屏幕截图。

        可选择截取当前视口或整个可滚动页面。
        可指定保存路径，否则保存到临时目录，并在浏览器关闭时自动清理。
        """
        # 1. 获取并验证页面
        ...
        page_id = params.page_id or await browser.get_active_page_id()
        if not page_id:
            return ToolResult(error="无法确定要截图的页面ID")

        # 2. 调用 MagicBrowser 的 take_screenshot 方法
        try:
            result = await browser.take_screenshot(
                page_id=page_id,
                path=params.path, # 可能为 None
            ...
            if isinstance(result, MagicBrowserError):
                return ToolResult(error=result.error)
            elif isinstance(result, ScreenshotSuccess):
                # 格式化成功结果
                screenshot_type = "整页" if params.full_page else "当前视口"
                save_location = f"保存路径: `{result.path}`"
                if result.is_temp:
                    save_location += " (临时文件，将在浏览器关闭时自动清理)"

                markdown_content = (
                    f"**操作: take_screenshot**\n"
                    f"状态: 成功 ✓\n"
                    f"类型: {screenshot_type}\n"
                    f"{save_location}"
                )
                # 将截图路径作为附件返回，以便前端展示
                return ToolResult(content=markdown_content, attachments=[str(result.path)])
            else:
                logger.error(f"take_screenshot 操作返回了未知类型: {type(result)}")
                return ToolResult(error="take_screenshot 操作返回了意外的结果类型。")

        except Exception as e:
            logger.error(f"take_screenshot 外部处理失败: {e!s}", exc_info=True)
            return ToolResult(error=f"截图时发生意外错误: {e!s}")
    ...
    async def visual_query(self, browser: MagicBrowser, params: VisualQueryParams) -> ToolResult:
        """使用视觉理解能力分析当前网页的视口内的内容，可配合 scroll_to 逐步分析整个网页的布局、风格、元素特征等，也可以用于理解有大量图片元素的网站。
        """
        # 1. 获取并验证页面
        page, error_result = await self._get_validated_page(browser, params)
        if error_result: return error_result
