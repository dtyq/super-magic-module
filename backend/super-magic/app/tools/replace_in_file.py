...
from app.utils.syntax_checker import SyntaxChecker
...
class ReplaceInFile(AbstractFileTool[ReplaceInFileParams], WorkspaceGuardTool[ReplaceInFileParams]):
    ...
    此工具可以使用特殊的 SEARCH/REPLACE 块格式对现有文件进行精确的部分修改，
    用于定义要修改的内容。SEARCH块中的内容必须与文件中目标位置的内容完全一致。
    """

    # 设置参数类
    params_class = ReplaceInFileParams

    # 设置工具元数据
    name = "replace_in_file"
    description = """文件内容精确替换工具，用于对现有文件进行精确的局部修改。
重要提示：仅使用 SEARCH/REPLACE 格式 - 请勿使用 git diff 格式。
SEARCH块中的内容必须与文件中的内容完全匹配（逐字符匹配，包括所有空格、换行和缩进）。
在使用此工具前，建议先使用read_file工具查看文件的准确内容。
请参考系统提示中的工具参考文档获取详细的使用说明和示例。""" if USE_CHINESE else """File Content Precise Replacement Tool for making targeted modifications to existing files.
...
>>>>>>> REPLACE"
    ...
    async def execute(self, tool_context: ToolContext, params: ReplaceInFileParams) -> ToolResult:
        ...
        try:
            ...
            # 构建新的文件内容并获取变更统计
            try:
                replace_result = await self._construct_new_file_content(params.diff, original_content)
    ...
    async def _construct_new_file_content(self, diff_content: str, original_content: str) -> ReplaceResult:
        """
        构建新的文件内容并统计变更情况
