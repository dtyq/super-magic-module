...
from app.schema import FileInfo
...
class FileSearch(WorkspaceGuardTool[FileSearchParams]):
    ...
    # 设置参数类
    params_class = FileSearchParams

    # 设置工具元数据
    name = "file_search"
    description = "基于对文件路径的模糊匹配的快速文件搜索。如果你知道文件路径的一部分但不确切知道它的位置，请使用此工具。响应将限制为10个结果。如果需要进一步过滤结果，请使查询更具体。"
