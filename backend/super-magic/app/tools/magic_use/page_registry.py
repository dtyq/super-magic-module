...
from app.tools.magic_use.js_loader import JSLoader
from app.tools.magic_use.userscript_manager import UserscriptManager
...
class PageRegistry:
    ...
    async def register_page(self, page: Page, context_id: Optional[str] = None) -> str:
        ...
        async with self._lock:
            ...
            # 监听页面事件
            # 页面DOM内容加载完成时执行核心JS加载和油猴脚本注入
            # 使用 lambda: asyncio.create_task(...) 确保异步执行且不阻塞事件循环
            page.on("domcontentloaded", lambda: asyncio.create_task(handle_page_load_and_script_injection()))
