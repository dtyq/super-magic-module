...
from app.tools.magic_use.browser_manager import BrowserManager
from app.tools.magic_use.magic_browser_config import MagicBrowserConfig
from app.tools.magic_use.page_registry import PageRegistry, PageState
    ...
    async def _wait_for_stable_network(self, page: Page, wait_time: float = 0.5, max_wait_time: float = 5.0):
        ...
        try:
            ...
            while asyncio.get_event_loop().time() - start_time < max_wait_time:
                # 等待网络空闲或者一小段时间
                try:
                    await asyncio.wait_for(network_idle_event.wait(), timeout=wait_time)
                    # 如果事件是设置状态，并且在等待后仍然是设置状态，则网络可能已稳定
                    if network_idle_event.is_set():
                        await asyncio.sleep(wait_time / 2) # 额外的小等待确认
                        if network_idle_event.is_set():
                            break
                except asyncio.TimeoutError:
                    # 超时说明在 wait_time 内有网络活动，继续循环
                    pass
                except asyncio.CancelledError:
                    break # 如果任务被取消，退出

            if asyncio.get_event_loop().time() - start_time >= max_wait_time:
                logger.debug(f"等待网络稳定超时 (>{max_wait_time}s)，仍有 {active_requests} 个活动请求")
