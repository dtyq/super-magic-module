...
class ListDir(WorkspaceGuardTool[ListDirParams]):
    ...
    这是用于发现的快速工具。在深入研究特定文件之前，尝试了解文件结构非常有用。可用于探索代码库。

    对于文本文件，会显示文件大小、行数和token数量，便于评估内容量。
    """

    # 设置参数类型
    params_class = ListDirParams
