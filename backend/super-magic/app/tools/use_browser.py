...
from app.tools.magic_use.magic_browser import MagicBrowser, MagicBrowserConfig, PageStateSuccess, ScreenshotSuccess, MagicBrowserError
...
class UseBrowser(WorkspaceGuardTool[UseBrowserParams], AbstractFileTool):
    ...
    # 设置参数类
    params_class = UseBrowserParams

    # 设置工具元数据
    name = "use_browser"
    description = """Browser Tool that provides atomic browser operations.
  ...
  <instructions>
            ...
            if isinstance(active_page_state_result, PageStateSuccess):
                ...
                if page_state.position_info and page_state.scroll_position:
                    ...
                    if scroll.document_height > 0 and scroll.viewport_height > 0:
                        ...
                        # 构建位置描述
                        position_desc = f"当前位置：第{current_screen:.0f}屏/共{total_screens:.0f}屏"

                        # 根据滚动位置提供相应的描述
                        if current_pos < viewport_height * 0.5:  # 第一屏
    ...
    async def get_tool_detail(self, tool_context: ToolContext, result: ToolResult, arguments: Dict[str, Any] = None) -> Optional[ToolDetail]:
        ...
        if event_context.attachments:
            try:
                # 获取当前页面的url
                browser: MagicBrowser = await tool_context.agent_context.get_resource("browser", self._create_browser)
                page_id = await browser.get_active_page_id()
                page = await browser.get_page_by_id(page_id)
                url = page.url
                title = await page.title()
