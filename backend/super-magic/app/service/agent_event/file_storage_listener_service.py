...
from app.core.entity.event.event import MainAgentFinishedEventData
...
class FileStorageListenerService:
    ...
    def register_standard_listeners(agent_context: AgentContext) -> None:
        ...
        event_listeners = {
            ...
            EventType.MAIN_AGENT_FINISHED: FileStorageListenerService._handle_main_agent_finished  # 新增事件监听
        }
    ...
    async def _handle_file_event(event: Event[FileEventData]) -> None:
        ...
        storage_response = await FileStorageListenerService._upload_file_to_storage(
            event.data.filepath,
            event.data.tool_context.agent_context
        )
        ...
        if storage_response:
            try:
                ...
                # 将附件添加到代理上下文
                FileStorageListenerService._add_attachment_to_agent_context(event.data.tool_context.agent_context, attachment)
    ...
    async def _handle_main_agent_finished(event: Event[MainAgentFinishedEventData]) -> None:
        ...
        Args:
            event: 主代理完成事件对象，包含 MainAgentFinishedEventData 数据
        """
        pass
    ...
    def _add_attachment_to_event_context(tool_context: ToolContext, attachment: Attachment) -> None:
        ...
        try:
            # 添加到事件上下文
            tool_context.event_context.add_attachment(attachment)
