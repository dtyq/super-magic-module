...
class TodoListenerService:
    ...
    def __init__(self):
        ...
        if self._snowflake_service is None:
            # 初始化雪花ID生成器
            TodoListenerService._snowflake_service = SnowflakeService.create_default()
    ...
    async def _handle_file_event(event: Event[FileEventData]) -> None:
        ...
        event_context = event.data.tool_context.event_context
    ...
    def _process_todo_items(todo_items: List[Dict[str, Any]], agent_context: AgentContext) -> None:
        ...
        # 直接替换代理上下文中的待办事项字典
        agent_context.agent_common_context._todo_items = new_todo_dict
