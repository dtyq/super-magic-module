...
from app.core.entity.event.event import AfterInitEventData
...
class AgentService:
    ...
    async def download_and_extract_workspace(self, agent_context: AgentContext) -> None:
        ...
        # 如果不需要更新工作区，则直接返回
        if not need_update or not Environment.is_support_fetch_workspace():
    ...
    async def init_workspace(
        ...
        tool_context = ToolContext(agent_context=agent_context)
    ...
    def create_agent_context(
        ...
        agent_context.set_llm_model(llm_model)
        ...
        # 如果提供了 agent_type，从文件中提取 llm_model
        if agent_type:
            # 读取 agent 文件内容
            ...
            if os.path.exists(agent_file):
                try:
                    ...
                    # 使用正则表达式直接从内容中提取模型名称
                    model_pattern = r"<!--\s*llm_model:\s*([a-zA-Z0-9-_.]+)\s*-->"
                    match = re.search(model_pattern, content, re.IGNORECASE)

                    if match:
                        model_name = match.group(1).strip()
                        agent_context.llm_model = model_name
                        logger.info(f"从 {agent_type}.agent 文件提取模型名称并设置为: {model_name}")
                except Exception as e:
                    logger.error(f"从 agent 文件提取 llm_model 时出错: {e}")
