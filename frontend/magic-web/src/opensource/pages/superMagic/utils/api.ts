...
export const getAttachmentsByThreadId = async ({ id }: { id: string }) => {
	return magicClient.post(`/api/v1/super-agent/topics/${id}/attachments`, {
		page: 1,
		page_size: 999,
...
export const getTemporaryDownloadUrl = async ({ file_ids }: { file_ids: string[] }) => {
	...
	const apiPath = isMagicShare
		? "/api/v1/super-agent/statistics/get-file-url"
...
export const getUsage = async ({
	...
	try {
		...
		const response = await magicClient.post("/api/v1/super-agent/statistics/user-usage", {
			page,
			page_size,
			organization_code,
			user_name,
...
export const getUsageMetrics = async () => {
	return magicClient.get("/api/v1/super-agent/statistics/topic-metrics")
}

// 获取所有工作区的组织列表
export const getUsageList = async () => {
	return magicClient.get("/api/v1/super-agent/workspaces/organization-codes")
}

// 获取话题沙箱状态
export const getTopicSandboxStatus = async ({ id }: { id: string }) => {
	return magicClient.get(`/api/v1/super-agent/topics/${id}/sandbox`)
}
