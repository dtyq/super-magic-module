...
import SuperMagicMobileWorkSpace from "@/opensource/pages/superMagicMobile/pages/workspace"
...
function MainWorkspaceContent() {
	...
	const updateAttachments = useCallback((selectedThreadInfo: any) => {
		if (!selectedThreadInfo?.id) return
		try {
	...
	const pullMessage = useCallback(
		async ({
			...
			const newMessage = res?.items
				.filter((item: any) => {
					return (
						item?.seq?.message?.general_agent_card || item?.seq?.message?.text?.content
					)
				})
				?.map((item: any) => {
			...
			const hasAttachments = newMessage.some(
				(item: any) => item?.attachments?.length > 0 || item?.tool?.attachments?.length > 0,
			)
			if (hasAttachments) {
			...
			setTopicMessagesMap((pre) => {
				...
				if (updatePageToken && page_token) {
					console.log(chat_topic_id, "topicNotHaveMoreMessageMap设置为", !res.has_more)
	...
	}, [selectedThreadInfo, showLoading, selectedWorkspace, setWorkspaces])
