 ...
 * 会话机器人数据管理器
 */
class ConversationBotDataManager {
	// 编辑器相关
	editorRef?: MagicRichEditorRef | null
	 ...
	 * @returns
	 */
	fetchBotInfo(userId: string) {
		// 如果用户ID为空，则不获取机器人信息
		if (!userId) return Promise.reject(new Error("用户ID为空"))
...
export default ConversationBotDataManager
