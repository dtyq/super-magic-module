...
describe('大型JSON转换测试', () => {
  ...
  // 读取大型测试数据
  beforeAll(() => {
    const filePath = path.resolve(__dirname, '../../components/FlowAssistant/all_flow_nodes.json');
  ...
  it('应能够将大型Flow JSON转换为YAML并保持结构完整', () => {
    ...
    // 写入文件以便手动检查
    const yamlOutputPath = path.resolve(__dirname, '../../components/FlowAssistant/all_flow_nodes.yaml');
    ...
    // 将转换后的JSON写入文件以便手动比较
    const jsonOutputPath = path.resolve(__dirname, '../../components/FlowAssistant/all_flow_nodes_converted.json');
  ...
  it('应能够处理节点中的复杂参数结构', () => {
    ...
    // 找一个具有复杂结构的节点进行比较
    const complexNodes = allFlowNodesJson.nodes.filter((n: any) => 
      n.params && (
    ...
    if (complexNodes.length > 0) {
      ...
      const convertedComplexNode = convertedJson.nodes.find((n: any) => n.id === originalComplexNode.id);
      ...
      if (originalComplexNode.params.branches) {
        ...
        expect(convertedComplexNode.params.branches.length).toBe(originalComplexNode.params.branches.length);
      ...
      if (originalComplexNode.params.model_config) {
        ...
        Object.keys(originalComplexNode.params.model_config).forEach(key => {
          expect(convertedComplexNode.params.model_config[key]).toBe(originalComplexNode.params.model_config[key]);
        });
      }
    }
  }, 30000); // 30秒超时
