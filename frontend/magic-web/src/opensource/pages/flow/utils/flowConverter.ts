      ...
      if (!json.nodes || !json.edges) {
        const defaultJson = {
          flow: {
            ...
            type: json.type ? (typeof json.type === 'number' ? 
                  (json.type === 1 ? 'workflow' : 
                   json.type === 2 ? 'knowledge' : 'default') : json.type) : 'default',
            icon: json.icon || '',
            enabled: json.enabled !== undefined ? json.enabled : true
          },
          variables: json.global_variable ? 
            (Array.isArray(json.global_variable) ? json.global_variable : []) : [],
          nodes: json.nodes || [],
          edges: json.edges || []
        };
