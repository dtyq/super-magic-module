...
import { Progress, Tooltip, Flex, Modal, Select } from "antd"
...
type TeamshareKnowledgeSelectProps = {
	value?: Knowledge.TeamshareKnowledgeItem
	onChange?: (value: Knowledge.TeamshareKnowledgeItem) => void
	options: DefaultOptionType[]
	progressList: Knowledge.TeamshareKnowledgeProgress[]
	setProgressList: Dispatch<SetStateAction<Knowledge.TeamshareKnowledgeProgress[]>>
	initInterval: () => void
}
...
export default function TeamshareKnowledgeSelectV1({
	...
	const createTeamshareKnowledgeVector = useMemoizedFn(async (businessId) => {
		setProgressList(
	...
	const confirmFn = useMemoizedFn(
		(selected: Knowledge.TeamshareKnowledgeItem, type: "new" | "retry") => {
			...
			Modal.confirm({
				...
				content,
				onOk: async () => {
					createTeamshareKnowledgeVector(selected.business_id)
				},
