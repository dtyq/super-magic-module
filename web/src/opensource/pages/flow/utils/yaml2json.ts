...
const restoreSchema = (fields: any[]): any => {
  ...
  fields.forEach((field, index) => {
    const fieldType = field.type && field.type.includes(':') 
      ? field.type.split(':')[0] 
      : field.type;
    
    const fieldItemType = field.type && field.type.includes(':')
      ? field.type.split(':')[1]
      : null;
...
const convertToFlowNode = (dslNode: any): Node => {
  const nodeType = getNodeTypeNumber(dslNode.type);
  const result = {
    id: dslNode.id,
    node_id: dslNode.id,
    node_type: nodeType,
    node_version: dslNode.version || 'v0',
    name: dslNode.name || '',
...
export const yaml2json = (yamlDSL: FlowDSL): Flow => {
  try {
    // 转换基本信息
    const flow: Flow = {
      id: yamlDSL.flow.id || `MAGIC-FLOW-${uuidv4().replace(/-/g, '')}-${Date.now().toString().slice(-8)}`,
