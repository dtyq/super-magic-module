...
describe('测试 FlowConverter', () => {
  ...
  describe('小型完整案例测试', () => {
    ...
    // 读取测试数据
    beforeEach(() => {
      const filePath = path.resolve(__dirname, '../../components/FlowAssistant/all_flow_nodes_small.json');
    ...
    it('应能将小型JSON对象转换为YAML并保持数据完整性', () => {
      ...
      if (smallJson.global_variable && jsonFromYaml.global_variable) {
        expect(jsonFromYaml.global_variable.variables[0].name).toBe(smallJson.global_variable.variables[0].name);
        expect(jsonFromYaml.global_variable.variables[0].default_value).toBe(smallJson.global_variable.variables[0].default_value);
      }
    });
    ...
    it('转换后的YAML应能转回JSON并与原始JSON匹配', () => {
      ...
      function checkObjectEquality(original: any, converted: any, path = '') {
        ...
        for (let i = 0; i < original.nodes.length; i++) {
          expect(converted.nodes[i].id, `${path}.nodes[${i}].id`).toBe(original.nodes[i].id);
          expect(converted.nodes[i].node_type, `${path}.nodes[${i}].node_type`).toBe(original.nodes[i].node_type);
        }
        
        // 检查边匹配
        expect(converted.edges.length, `${path}.edges.length`).toBe(original.edges.length);
        for (let i = 0; i < original.edges.length; i++) {
          expect(converted.edges[i].source, `${path}.edges[${i}].source`).toBe(original.edges[i].source);
          expect(converted.edges[i].target, `${path}.edges[${i}].target`).toBe(original.edges[i].target);
        }
      }
