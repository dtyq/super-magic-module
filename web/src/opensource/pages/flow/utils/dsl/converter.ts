...
export const dsl2json = (dslString: string): Flow => {
  try {
    ...
    const flow: Flow = {
      ...
      icon: dsl.app.icon || 'https://teamshareos-app-public.tos-cn-beijing.volces.com/YAML/713471849556451329/default/bot.png',
    ...
    throw new Error(`转换DSL到JSON失败: ${error instanceof Error ? error.message : String(error)}`);
  }
};
...
export const jsonStr2json = (jsonString: string): Flow => {
  try {
    ...
    throw new Error(`解析JSON字符串失败: ${error instanceof Error ? error.message : String(error)}`);
  }
};
...
const generateDependencies = (flow: Flow): any[] => {
  ...
  flow.nodes.forEach(node => {
    ...
    // 检查是否有工具节点使用Google
    if (node.node_type === customNodeType.Tools && 
        node.params.provider_id === 'google') {
      // 添加Google依赖
...
export const json2dsl = (flow: Flow): DSL => {
  try {
    ...
    throw new Error(`转换JSON到DSL失败: ${error instanceof Error ? error.message : String(error)}`);
  }
};
...
export const json2dslString = (flow: Flow): string => {
  try {
    ...
    throw new Error(`转换JSON到DSL字符串失败: ${error instanceof Error ? error.message : String(error)}`);
  }
};
...
export const jsonStr2dslString = (jsonString: string): string => {
  try {
    ...
    throw new Error(`转换JSON字符串到DSL字符串失败: ${error instanceof Error ? error.message : String(error)}`);
  }
};
